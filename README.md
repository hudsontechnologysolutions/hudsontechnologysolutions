# Hi, I'm Roberto

My mission is simple: improve the lives of others.

I'm a developer who creates software and websites that help non-profits & small businesses save time, scale their impact, & increase their online presence.

Currently:
- Software Engineer Consultant at Hudson Technology Solutions (Founder)
- Software Engineer / Data Scientist at Public Service Enterprise Group

Focused on:
- Full-stack applications
- Websites
- AI-powered automation tools

---

# Connect With Me

- LinkedIn: https://linkedin.com/in/roberto-herndon
- Email: roberto@hudsontech.dev
- Articles, Blog, Content, and Links: https://linktr.ee/hudsontech

---

# Tech Stack

## Frontend
React • Tailwind CSS • HTML

## Backend
Python • Node.js • FastAPI • Express.js

## Cloud & Infrastructure
AWS • Vercel • Render

## Data & APIs
MongoDB • SQL • REST APIs

## AI Tools
Claude • Microsoft Copilot

---

# Featured Projects

## 🏠 Houses4Us
Automation platform for generating Affirmative Fair Housing Marketing Plan demographic reports using Census and HUD datasets.

**Tech:** React, FastAPI, U.S. Census API, Vercel

- Automated demographic reporting workflows
- Eliminated manual Census lookup processes
- Built for affordable housing professionals in New Jersey

[Project](https://houses4us.vercel.app/) | [Repository](https://github.com/hudsontechnologysolutions/houses4us)

---

## 🎨 Muse Map
Subscription-based cultural event platform for artists and nonprofits.

**Tech:** React, Node.js, MongoDB, Stripe

- Interactive map + calendar platform
- Stripe-integrated subscription billing
- Currently supporting real paying users

[Project](https://muse-map.vercel.app/) | [Repository](https://github.com/hudsontechnologysolutions/muse-map)

---

## 🎵 WhereRTheBands
Live music discovery platform helping users, and myself (I love music), find local bands, artists, and music events.

**Tech:** React, Vercel, Modern Frontend Web Technologies

- Discover local music events and performances
- Built responsive UI/UX for event exploration
- Focused on supporting local artists and music communities

[Project](https://whererthebands.vercel.app/) | [Repository](https://github.com/hudsontechnologysolutions/whererthebands)

---

# Experience

## Hudson Technology Solutions — Founder (Software Engineer Consultant)
Founder of a software consultancy agency that creates software and websites that help non-profits & small businesses save time, scale their impact, & increase their online presence.

## PSEG — Software Engineer / Data Scientist
Building production-grade Python backend systems and enterprise data integrations.

## Poweroptions — Software Engineer Intern  - Data
Created Python scripts to generate emissions data to reduce the carbon footprint of municipalities & other civic organizations in New Englad.

## Pinterest — Software Engineer Intern - Full-Stack
Worked on internal observability tooling using React and Flask at the headquarters in San Francisco.

## Hack4Impact — President
President of a club dedicated to building software applications for nonprofit organizations to advance their mission.



